...
class hmm:
    ...
    def get_current_state(self, states_seq=None):   
        if states_seq != None:
           return states_seq[-1]         # The last state in the sequence
        else:
           (prob, curr_state) = max((self.pi[s], s) for s in self.pi)
        return curr_state                # The state with the highest init. prob.
